...
% Initialization
Populations = CreateEmptyParticle(Population_num);

for i = 1:Population_num
    Populations(i).Position = zeros(1,nVar);
    for j = 1:nVar
        Populations(i).Position(1,j) = unifrnd(min_range(j),max_range(j),1);
    end
    Populations(i).Cost = fobj(Populations(i).Position, fopt, PV);
    Populations(i).Best.Position = Populations(i).Position;
    Populations(i).Best.Cost = Populations(i).Cost;
end

Populations = DetermineDomination(Populations);

Archive = GetNonDominatedParticles(Populations);

costs = GetCosts(Populations);
...
for it = 1:MaxIt
    ...
    parfor i = 1:Population_num
        ...
        Ceq = Alpha.Position;
        G0 = GCP.*(Ceq-lambda.*Populations(i).Position);        % Eq(14)
        G = G0.*F;                                              % Eq(13)
        Position_Alpha = Ceq + ((Populations(i).Position-Ceq).*F...
            + (G./lambda*V).*(1-F));                            % Eq(16) for ALPHA
        ...
        G0 = GCP.*(Ceq-lambda.*Populations(i).Position);        % Eq(14)
        G = G0.*F;                                              % Eq(13)
        Position_Beta = Ceq + ((Populations(i).Position-Ceq).*F...
            + (G./lambda*V).*(1-F));                            % Eq(16) for BETA
        ...
        G0 = GCP.*(Ceq-lambda.*Populations(i).Position);        % Eq(14)
        G = G0.*F;                                              % Eq(13)
        Position_Delta = Ceq + ((Populations(i).Position-Ceq).*F...
            + (G./lambda*V).*(1-F));                            % Eq(16) for DELTA
        
        % Global
        Populations(i).Position = (Position_Alpha + Position_Beta + Position_Delta)/3;
        
        % Boundary checking
        Populations(i).Position = min(max(Populations(i).Position,min_range),max_range);
        
        % Eq.(3.4) in the paper
        %         c = 2.*rand(1, nVar);
        %         % Eq.(3.1) in the paper
        %         D = abs(c.*Delta.Position-Populations(i).Position);
        %         % Eq.(3.3) in the paper
        %         A = 2.*a.*rand(1, nVar)-a;
        %         % Eq.(3.8) in the paper
        ...
        %         c = 2.*rand(1, nVar);
        %         % Eq.(3.1) in the paper
        %         D = abs(c.*Beta.Position-Populations(i).Position);
        %         % Eq.(3.3) in the paper
        %         A = 2.*a.*rand()-a;
        %         % Eq.(3.9) in the paper
        ...
        %         c = 2.*rand(1, nVar);
        %         % Eq.(3.1) in the paper
        %         D = abs(c.*Alpha.Position-Populations(i).Position);
        %         % Eq.(3.3) in the paper
        %         A = 2.*a.*rand()-a;
        ...
        %         Populations(i).Position = (X1+X2+X3)./3;
    ...
    Populations = DetermineDomination(Populations);
    non_dominated_wolves = GetNonDominatedParticles(Populations);
